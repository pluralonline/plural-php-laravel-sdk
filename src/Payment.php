    ...
    {        
        try {
            ...
            if ((isset($response['response_code']) && $response['response_code'] != 1) || (isset($response['respone_code']) && $response['respone_code'] != 1)) {
                throw new \Exception(($response['response_message'] ?? ($response['respone_message'] ?? 'Something went wrong')), ($response['response_code'] ?? ($response['respone_code'] ?? '')));
            }
    ...
    public function Fetch($txnID = null, $txnType = 3)
    {
        try {
            ...
            if(!isset($response['ppc_ParentTxnResponseCode'])){
                throw new \Exception(($response['ppc_TxnResponseMessage'] ?? 'Something went wrong'), ($response['ppc_TxnResponseCode'] ?? ''));
            }
